...
//	DoubleJumpPowerup doubleJPow;
    ...
    @Override
    public Player(int x, int y, int width, int height, FlxGroup Bullets,FlxEmitter Gibs, FlxVirtualPad pad) {//FlxVirtualPad pad
        super(width, height);
		this.pad = pad;	
		isUpsideDown = false;
		isSidewaysm = false;
...
/*		doubleJPowTimer = new FlxTimer();
	...
	*/
	  ...
	  //	  loadGraphic("jose.png",true,true,16,16);
		width = 12;//14
		height = 12;//15
	/*	_joystick.onPressed = move;
		_joystick.onUp = stop;
...
//		offset.x = 1;
	...
	*/
		...
		if(FlxG.keys.A || pad.buttonLeft.status == FlxButton.PRESSED && alive)
		{
		...
		else if(FlxG.keys.D || pad.buttonRight.status == FlxButton.PRESSED && alive)
		{
				 if(isTouching(FlxObject.FLOOR))
			 if(!isUpsideDown){
			 	 play("walk");}
	...
	//		setFacing(RIGHT);
		...
		if(FlxG.keys.W || pad.buttonA.status == FlxButton.PRESSED && alive)
		{
			if(!isUpsideDown) {
				play("jump");
			}
	...
	/*	if(FlxG.keys.UP  || pad.buttonUp.status == FlxButton.PRESSED)
		 ...
		 if((FlxG.keys.SPACE || pad.buttonB.status == FlxButton.PRESSED)&& _bullets.countLiving() < 3 && alive)
			 {		
				 if(!_justShoot)
				 {
					 if(getFlickering())
	...
	{		
		...
		public void callback()
		{
			...
			velocity.x = 3 * _joystick.acceleration.x;//velocity.x
			if(velocity.x < 0){
				setFacing(LEFT);
